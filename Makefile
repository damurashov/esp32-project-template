...
PATH_XTS_COMPILER = $(wildcard $(PATH_IDF_TOOLS_PATH)/tools/xtensa-esp32-elf/*esp*)/xtensa-esp32-elf/bin
...
IS_NEW_COMPONENT = $(filter new_component,$(MAKECMDGOALS))

ifneq ($(IS_NEW_COMPONENT),)
  ifeq ($(COMPONENT_FRAME),)
    $(error Missing parameter COMPONENT_FRAME)
  endif

  ifeq ($(COMPONENT_FRAME_NAMESPACE),)
    $(error Missing parameter COMPONENT_FRAME_NAMESPACE)
  endif

  ifeq ($(COMPONENT_FRAME_CLASS),)
    $(error Missing parameter COMPONENT_FRAME_CLASS)
  endif
endif
...
new_component:
	...
	cp tools/component_frame/component_frame.cpp components/$(COMPONENT_FRAME)/$(COMPONENT_FRAME)/$(COMPONENT_FRAME_CLASS).cpp
	cp tools/component_frame/component_frame.hpp components/$(COMPONENT_FRAME)/$(COMPONENT_FRAME)/$(COMPONENT_FRAME_CLASS).hpp
	cp tools/component_frame/component_globals.hpp components/$(COMPONENT_FRAME)/$(COMPONENT_FRAME)/$(COMPONENT_FRAME).hpp
	...
	export COMPONENT_FRAME_UPPER=`echo $(COMPONENT_FRAME) | tr a-z A-Z` \
		&& find components/$(COMPONENT_FRAME) -type f | xargs -n 1 sed -i "s/COMPONENT_FRAME_UPPER/$${COMPONENT_FRAME_UPPER}/g"
	find components/$(COMPONENT_FRAME) -type f | xargs -n 1 sed -i 's/COMPONENT_FRAME_NAMESPACE/$(COMPONENT_FRAME_NAMESPACE)/g'
	find components/$(COMPONENT_FRAME) -type f | xargs -n 1 sed -i 's/COMPONENT_FRAME_CLASS/$(COMPONENT_FRAME_CLASS)/g'
